...
parser.add_argument('--filename',default=None,help='record filename (default: com.package.name_yyyyddmm_hhmmss)')
parser.add_argument('--record-format', default='mkv', choices='mp4 mkv m4a mka opus aac flac wav'.split(),help='record format (default: mkv)')
...
debug.debug('record_format:',record_format)
...
record_command=libscrrec.set_record(path,filename,require_audio=require_audio,no_audio_playback=no_audio_playback,no_video_playback=no_video_playback,no_control=control,
	video_codec=video_codec,video_encoder=video_encoder,audio_codec=audio_codec,audio_encoder=audio_encoder, record_format=record_format)
